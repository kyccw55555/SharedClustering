############################################################################################################################################
##	"Shared Clustering" parameter sampling -- speed improved version for vectorial data only
...
(X,u0,A,v0,T,a,px,C,E,u)	
{
	...
	dc<-rep(0,N)
	for (index in 1:N){
		sampc<-rcompsCx(index,X[index,],px,u,E,C)
		C[index]<-sampc$label
		judge<-validc(K,C)
		if (judge$ind!=1)
			C[index]<-judge$miss
	...
	out<-NULL
	out$px<-px; out$dpx<-dpx; out$C<-C; out$dc<-dc; out$E<-E; out$u<-u; 
	out
}

jopolikx<-function	#Calculate joint posterior likelihood at one stage
(dat,u,E,v0,T,u0,A,C) 
{	
...
(dat,cnt=2000,burnin=1000)
{
	...
	for(rep in 1:cnt){
		one<-Gibbsonex(dat$X,u0,A,v0,T,a,px,C,E,u)
		E<-one$E
		u<-one$u
		px<-one$px
		C<-one$C
		...
		#cat("#",rep,"Success!\n")
		plotlik[rep]<-jopolikx(dat,u,E,v0,T,u0,A,C)+log(one$dpx)+sum(log(one$dc))
	...
	#plot(plotlik,type="l",main="Joint Posterior Log-Likelihood -- vector data",xlab="cnt",ylab="value")
	res<-NULL
	res$px<-px_pmode; res$C<-C_pmode; res$E<-E_pmode; res$u<-u_pmode;
	#cat("sample finished.\n")
	res
}
